...
describe('ErrorNotificationService', () => {
    ...
    beforeEach(async () => {
        ...
        // Mock dataSource
        dataSource_1.createDataSource.mockResolvedValue({
        ...
        error_mapping_service_1.ErrorMappingService.mockImplementation(() => mockErrorMappingService);
    ...
    describe('createFromModbus()', () => {
        it('should create notification from Modbus error', async () => {
            ...
            const result = await errorNotificationService.createFromModbus(source, 'Climate_Controller', 'device_001');
            expect(result).not.toBeNull();
            expect(result === null || result === void 0 ? void 0 : result.err_code).toBe('ERR_TEMP_HIGH');
            expect(mockErrorMappingService.parseModbusError).toHaveBeenCalledWith(source, 'Climate_Controller');
        ...
        it('should update existing notification instead of creating duplicate', async () => {
            ...
            mockNotificationRepo.update.mockResolvedValue({});
            mockNotificationRepo.findOne
                .mockResolvedValueOnce(existingNotification) // First call for deduplication
                .mockResolvedValueOnce(Object.assign(Object.assign({}, existingNotification), { metadata: JSON.stringify({
                    occurrence_count: 2,
                    first_occurred: '2025-01-17T10:00:00.000Z',
                    last_occurred: '2025-01-17T10:05:00.000Z'
                }) })); // Second call for fetching updated
            const source = {
                register_type: 'holding',
                address: 1000,
                value: 1
            ...
            const metadata = JSON.parse(updateCall[1].metadata);
    ...
    describe('autoResolveIfClear()', () => {
        it('should auto-resolve when error clears for holding register', async () => {
            ...
            const result = await errorNotificationService.autoResolveIfClear(source, 'Climate_Controller', 'device_001');
    ...
    describe('Edge Cases', () => {
        it('should handle malformed metadata gracefully', async () => {
            ...
            await expect(errorNotificationService.createFromModbus(source, 'Test', 'device_001')).resolves.not.toThrow();
        });
