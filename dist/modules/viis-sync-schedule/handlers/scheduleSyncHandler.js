...
exports.ScheduleSyncHandler = void 0;
...
const schedulePullPolicy_1 = require("./schedulePullPolicy");
/**
...
class ScheduleSyncHandler {
     ...
     */
    constructor(dbService, node, accessToken) {
        ...
        this.resetSyncStats();
        logger_1.logger.info(node, 'ScheduleSyncHandler initialized');
    }
    /**
    ...
    async syncAll() {
        ...
        try {
            ...
            logger_1.logger.info(this.node, `Synchronization completed successfully: Created ${this.syncStats.plansCreated} plans, ` +
            ...
            logger_1.logger.error(this.node, `Synchronization failed: ${errorMessage}`);
            // Record failed sync
     ...
     */
    async syncSchedulePlan(serverPlan) {
        try {
            logger_1.logger.info(this.node, `Syncing schedule plan: ${serverPlan.name} (${serverPlan.label})`);
            ...
            logger_1.logger.info(this.node, `Completed sync for schedule plan: ${serverPlan.name}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error(this.node, `Failed to sync schedule plan ${serverPlan.name}: ${errorMessage}`);
            throw error;
        }
    }
    /**
     ...
     */
    async createSchedulePlan(serverPlan) {
        try {
            logger_1.logger.info(this.node, `Creating new schedule plan: ${serverPlan.name}`);
            // Create a new TabiotSchedulePlan entity
            const newPlan = new TabiotSchedulePlan_1.TabiotSchedulePlan();
            newPlan.name = serverPlan.name;
            newPlan.label = serverPlan.label;
            newPlan.creation = (0, schedulePullPolicy_1.localInstantFromServer)(serverPlan.creation) || new Date();
            newPlan.modified = (0, schedulePullPolicy_1.localInstantFromServer)(serverPlan.modified) || new Date();
            ...
            logger_1.logger.info(this.node, `Created schedule plan: ${serverPlan.name}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error(this.node, `Failed to create schedule plan ${serverPlan.name}: ${errorMessage}`);
            throw error;
        }
    }
    /**
    ...
    async updateSchedulePlanIfNeeded(localPlan, serverPlan) {
        try {
            if ((0, schedulePullPolicy_1.shouldSkipIncomingUpdate)(localPlan, serverPlan.modified)) {
                logger_1.logger.info(this.node, `Local plan ${localPlan.name} is newer or same as server, skipping update`);
                return;
            }
            logger_1.logger.info(this.node, `Updating schedule plan: ${serverPlan.name}`);
            const incomingModified = (0, schedulePullPolicy_1.localInstantFromServer)(serverPlan.modified);
            ...
            logger_1.logger.info(this.node, `Updated schedule plan: ${serverPlan.name}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error(this.node, `Failed to update schedule plan ${serverPlan.name}: ${errorMessage}`);
            throw error;
        }
    }
    /**
     ...
     */
    async syncSchedulesForPlan(serverPlan) {
        try {
            const { schedules } = serverPlan;
            if (!schedules || !Array.isArray(schedules)) {
                logger_1.logger.warn(this.node, `No schedules found for plan ${serverPlan.name}`);
                return;
            }
            logger_1.logger.info(this.node, `Syncing ${schedules.length} schedules for plan ${serverPlan.name}`);
            ...
            logger_1.logger.info(this.node, `Completed syncing schedules for plan ${serverPlan.name}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error(this.node, `Failed to sync schedules for plan ${serverPlan.name}: ${errorMessage}`);
            throw error;
        }
    }
    /**
     ...
     */
    async syncSchedule(serverSchedule, planName) {
        try {
            ...
            logger_1.logger.info(this.node, `Completed sync for schedule: ${serverSchedule.name}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error(this.node, `Failed to sync schedule ${serverSchedule.name}: ${errorMessage}`);
            throw error;
        }
    }
    /**
     ...
     */
    async createSchedule(serverSchedule, planName) {
        try {
            ...
            newSchedule.creation = (0, schedulePullPolicy_1.localInstantFromServer)(serverSchedule.creation) || new Date();
            newSchedule.modified = (0, schedulePullPolicy_1.localInstantFromServer)(serverSchedule.modified) || new Date();
            await this.scheduleRepo.save(newSchedule);
            this.syncStats.schedulesCreated++;
            this.syncStats.totalSchedules++;
            logger_1.logger.info(this.node, `Created schedule: ${serverSchedule.name}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error(this.node, `Failed to create schedule ${serverSchedule.name}: ${errorMessage}`);
            throw error;
        }
    }
    /**
     ...
     */
    async updateScheduleIfNeeded(localSchedule, serverSchedule) {
        try {
            if ((0, schedulePullPolicy_1.shouldSkipIncomingUpdate)(localSchedule, serverSchedule.modified)) {
            ...
            const incomingModified = (0, schedulePullPolicy_1.localInstantFromServer)(serverSchedule.modified);
            ...
            this.syncStats.totalSchedules++;
            logger_1.logger.info(this.node, `Updated schedule: ${serverSchedule.name}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error(this.node, `Failed to update schedule ${serverSchedule.name}: ${errorMessage}`);
            throw error;
        }
    }
    /**
     ...
     */
    async detectDeletedSchedules(serverSchedules, planName) {
        try {
            logger_1.logger.info(this.node, `Checking for deleted schedules in plan ${planName}`);
            ...
            if (!localSchedules || localSchedules.length === 0) {
                logger_1.logger.info(this.node, `No local schedules found for plan ${planName}`);
                return;
            }
            ...
            if (deletedSchedules.length === 0) {
                logger_1.logger.info(this.node, `No deleted schedules found for plan ${planName}`);
                return;
            }
            logger_1.logger.info(this.node, `Found ${deletedSchedules.length} deleted schedules for plan ${planName}`);
            // Mark each missing schedule as deleted
            for (const deletedSchedule of deletedSchedules) {
                if (!(0, schedulePullPolicy_1.shouldSoftDeleteLocalOnPull)(deletedSchedule)) {
                    logger_1.logger.info(this.node, `Keeping unsynced local schedule ${deletedSchedule.name}`);
                    continue;
                }
            ...
            logger_1.logger.info(this.node, `Processed ${deletedSchedules.length} deleted schedules for plan ${planName}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.logger.error(this.node, `Failed to detect deleted schedules for plan ${planName}: ${errorMessage}`);
            throw error;
        }
    }
}
exports.ScheduleSyncHandler = ScheduleSyncHandler;
