...
module.exports = function (RED) {
    ...
    function ViisAisTelemetryNode(config) {
        ...
        let pendingPayloadToSave = null;
        let lastSavedTimestamp = 0;
        node.log(`[AIS] Initializing - Host: ${aisHost}:${aisPort}, Output interval: ${outputInterval}ms`);
        async function ensureDbInitialized() {
            if (dbInitFailed || dbInitStarted)
                return;
            const now = Date.now();
            if (dbInitNextRetryAt && now < dbInitNextRetryAt)
                return;
            dbInitStarted = true;
            try {
                ...
                telemetryRepo = dataSource.getRepository(TabiotDeviceTelemetry_1.TabiotDeviceTelemetry);
                ...
                node.warn(`[AIS] Failed to initialize database for telemetry storage: ${err.message}`);
                return;
            }
            finally {
                dbInitStarted = false;
        ...
        function buildTelemetryEntities(timestamp, payload) {
            const entities = [];
        ...
        async function savePayloadToDatabase(payload) {
            ...
            try {
                ...
                node.warn(`[AIS] Failed to save AIS telemetry to database: ${err.message}`);
            }
            finally {
                isSavingToDb = false;
