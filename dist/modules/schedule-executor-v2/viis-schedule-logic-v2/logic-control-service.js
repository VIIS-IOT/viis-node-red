...
exports.LogicControlService = void 0;
...
class LogicControlService {
    ...
    async readControlMode() {
        try {
            ...
            this.node.warn(`Error reading control mode: ${error.message}`);
    ...
    async readSafetyConditions() {
        try {
            ...
            this.node.warn(`Error reading safety conditions: ${error.message}`);
     ...
     */
    hasCommandOverlap(newCommands, activeCommands) {
    ...
    applyManualOverrides(commands) {
        ...
        return commands.map(cmd => {
            ...
            if (manualOverrides[overrideKey]) {
                ...
                return Object.assign(Object.assign({}, cmd), { value: override.value });
            }
            return cmd;
        });
    }
     ...
     */
    async checkBeforeExecute(input) {
        ...
        const activeCommands = this.node.context().global.get('activeModbusCommands') || {};
    ...
    async handleRpcSetMode(mode) {
        try {
            ...
            this.node.error(`Failed to set control mode: ${error.message}`);
            return false;
        }
    }
    /**
     ...
     */
    clearManualOverride(fc, address) {
        const globalContext = this.node.context().global;
        const manualOverrides = globalContext.get('manualModbusOverrides') || {};
...
exports.LogicControlService = LogicControlService;
