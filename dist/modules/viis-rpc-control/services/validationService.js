...
class ValidationService {
    constructor(options, configService) {
        ...
        this.logger = new logger_1.Logger(options.node, "VALIDATION-SERVICE");
    }
    /**
     * Validate and convert a value based on its expected type
     */
    validateAndConvertValue(key, value) {
     ...
     */
    convertValueByType(key, value, expectedType) {
        try {
            switch (expectedType) {
                case "number":
            ...
            const errorMessage = constants_1.ERROR_MESSAGES.VALUE_CONVERSION_FAILED(key) + `: ${error.message}`;
     ...
     */
    convertToNumber(key, value) {
        if (typeof value === "number") {
            if (isNaN(value) || !isFinite(value)) {
                throw new Error(constants_1.ERROR_MESSAGES.INVALID_NUMBER(key));
            }
            return value;
        }
     ...
     */
    validateAndConvertValues(values) {
        ...
        for (const [key, value] of Object.entries(values)) {
            try {
                ...
                this.logger.error(`Failed to validate value for key ${key}: ${error.message}`);
                throw error;
            }
        }
