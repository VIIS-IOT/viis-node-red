...
describe("resolvePollerConfig", () => {
    it("resolves JSON-string global config using the required priority order", () => {
        ...
        const resolved = (0, config_resolver_1.resolvePollerConfig)({
            boardId: "",
    ...
    it("falls back to legacy global keys, board-specific mappings, uppercase device id, and board1", () => {
        ...
        const resolved = (0, config_resolver_1.resolvePollerConfig)({ scaleConfigOverrides: "" }, global);
    ...
    it("uses unknown_device when no device id is present", () => {
        ...
        expect((0, config_resolver_1.resolvePollerConfig)({}, global).deviceId).toBe("unknown_device");
    });
    it("throws a clear error for invalid JSON config strings", () => {
        const global = makeGlobal({
            modbusPollGroups: "{broken",
        });
        expect(() => (0, config_resolver_1.resolvePollerConfig)({}, global)).toThrow("Invalid JSON in global context key modbusPollGroups");
    });
    it("throws a clear error when scale config overrides are not an array", () => {
        ...
        expect(() => (0, config_resolver_1.resolvePollerConfig)({ scaleConfigOverrides: '{"key":"current_ec"}' }, global)).toThrow("node config scaleConfigOverrides must be an array");
    });
    it("partitions global poll groups across every board in modbusBoards even when node boardId is board1", () => {
        ...
        const resolved = (0, config_resolver_1.resolvePollerConfig)({ boardId: "board1" }, global);
