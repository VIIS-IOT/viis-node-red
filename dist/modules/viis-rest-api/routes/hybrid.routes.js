...
/**
 * Hybrid routing setup that integrates routing-controllers with Node-RED
 */
class HybridRoutes {
    ...
    async setupRoutingControllers(RED) {
        try {
            ...
            const app = (0, routing_controllers_1.useExpressServer)(RED.httpNode, {
                // Route configuration - use different prefix to avoid conflicts
                routePrefix: `${this.configManager.get('apiPrefix')}/hybrid`,
                // Controllers to register
                controllers: [health_controller_1.HealthController, auth_controller_1.AuthController],
    ...
    setupCustomErrorHandling(RED) {
        RED.httpNode.use(`${this.configManager.get('apiPrefix')}/hybrid`, (error, req, res, next) => {
    ...
    logRegisteredRoutes() {
        if (this.configManager.isEnabled('enableDebugMode')) {
            logger_1.logger.info(this.node, 'routing-controllers routes registered:', {
                prefix: `${this.configManager.get('apiPrefix')}/hybrid`,
    ...
    getStatus() {
        return {
            enabled: true,
            routePrefix: `${this.configManager.get('apiPrefix')}/hybrid`,
            controllersRegistered: ['HealthController', 'AuthController'],
...
exports.HybridRoutes = HybridRoutes;
