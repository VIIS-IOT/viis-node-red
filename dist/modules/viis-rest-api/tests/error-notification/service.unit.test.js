...
describe('ErrorNotificationApiService - Unit Tests', () => {
    ...
    describe('createNotification', () => {
        it('should create a notification successfully', async () => {
            const testData = test_config_1.testNotificationData.valid;
            const mockNotification = Object.assign(Object.assign({ name: 'notification_test_001' }, testData), { is_read: 0, created_at: new Date() });
        ...
        it('should throw error when repository not initialized', async () => {
            service.notificationRepo = null;
            await expect(service.createNotification(test_config_1.testNotificationData.valid)).rejects.toThrow(routing_controllers_1.BadRequestError);
        });
    ...
    describe('getNotificationByName', () => {
        ...
        it('should throw NotFoundError when notification not found', async () => {
            ...
            await expect(service.getNotificationByName('non_existent')).rejects.toThrow(routing_controllers_1.NotFoundError);
        });
    });
    describe('getAllNotifications', () => {
        ...
        it('should filter by err_code', async () => {
            ...
            const whereClause = mockRepository.findAndCount.mock.calls[0][0].where;
    ...
    describe('updateNotification', () => {
        it('should update notification message', async () => {
            ...
            mockRepository.findOne.mockResolvedValue(mockNotification);
            mockRepository.save.mockResolvedValue(Object.assign(Object.assign({}, mockNotification), { message: 'New message' }));
        ...
        it('should mark notification as read', async () => {
            ...
            const metadata = JSON.parse(mockNotification.metadata);
        ...
        it('should merge metadata when updating', async () => {
            ...
            const updatedMetadata = JSON.parse(mockNotification.metadata);
    ...
    describe('deleteNotification', () => {
        ...
        it('should throw NotFoundError when deleting non-existent notification', async () => {
            mockRepository.findOne.mockResolvedValue(null);
            await expect(service.deleteNotification('non_existent')).rejects.toThrow(routing_controllers_1.NotFoundError);
        });
    });
    describe('bulkResolve', () => {
        it('should resolve multiple notifications', async () => {
            ...
            mockRepository.save.mockResolvedValue(mockNotifications);
        ...
        it('should filter by err_code when bulk resolving', async () => {
            ...
            expect(findOptions.where.err_code).toBe('TEST_ERR');
        });
        it('should only resolve unread notifications', async () => {
            mockRepository.find.mockResolvedValue([]);
            await service.bulkResolve({});
            const findOptions = mockRepository.find.mock.calls[0][0];
            expect(findOptions.where.is_read).toBe(0);
        });
