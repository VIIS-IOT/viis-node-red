...
class ModbusService {
     ...
     */
    async writeToModbusBoard(key, mapping, value, boardId) {
        try {
            let writeValue = value;
            // Apply scaling for numeric values
            if (typeof writeValue === "string" && !isNaN(Number(writeValue))) {
            ...
            const errorMsg = `Failed to write ${key} to ${boardId}: ${error.message}`;
            this.logger.error(errorMsg);
            throw new Error(errorMsg);
        }
    }
     ...
     */
    async readFromModbusBoard(key, mapping, boardId) {
        try {
            // Get board-specific client
            const modbusClient = await this.getModbusClient(boardId);
            const readFc = this.getReadFunctionCode(mapping.fc);
            ...
            const errorMsg = `Failed to read ${key} from ${boardId}: ${error.message}`;
            this.logger.error(errorMsg);
            throw new Error(errorMsg);
        }
    }
