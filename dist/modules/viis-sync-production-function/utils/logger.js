"use strict";
/**
 * @fileoverview Enhanced Logger utility for VIIS Sync Production Function module
 * Provides structured logging with detailed API and operation tracking
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.logger = void 0;
/**
 * Enhanced logger utility for production function sync node
 */
exports.logger = {
    /**
    ...
    info(node, message) {
        ...
        const logMessage = `[VIIS Sync Production Function] [${timestamp}] INFO: ${message}`;
        if (node && node.log) {
            node.log(logMessage);
        }
        else {
    ...
    warn(node, message) {
        const timestamp = new Date().toISOString();
        const logMessage = `[VIIS Sync Production Function] [${timestamp}] WARNING: ${message}`;
        if (node && node.warn) {
    ...
    error(node, message) {
        const timestamp = new Date().toISOString();
        const logMessage = `[VIIS Sync Production Function] [${timestamp}] ERROR: ${message}`;
        if (node && node.error) {
    ...
    debug(node, message, showDetailedLogs) {
        if (showDetailedLogs) {
            const timestamp = new Date().toISOString();
            const logMessage = `[VIIS Sync Production Function] [${timestamp}] DEBUG: ${message}`;
            if (node && node.debug) {
    ...
    errorWithStack(node, message, error, showDetailedLogs = false) {
        const timestamp = new Date().toISOString();
        const errorMessage = `[VIIS Sync Production Function] [${timestamp}] ERROR: ${message} - ${error.message}`;
        if (node && node.error) {
        ...
        if (showDetailedLogs && error.stack) {
            const stackMessage = `[VIIS Sync Production Function] [${timestamp}] STACK TRACE: ${error.stack}`;
            if (node && node.debug) {
