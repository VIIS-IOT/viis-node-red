...
class ScheduleHandler {
    ...
    async handleGet(path, query, msg) {
    ...
    async handlePost(path, msg) {
        ...
        try {
            ...
            try {
                ...
                await (0, outboxAck_1.markOutboxSynced)(this.scheduleRepo, 'tabiot_schedule', savedSchedule.name);
    ...
    async handlePut(path, msg) {
            ...
            try {
                ...
                if (!modbusClient) {
                    ...
                    try {
                        ...
                        const thingsboardClient = await client_registry_1.default.getThingsboardMqttClient(thingsboardConfig, this.node);
                        const emqxClient = await client_registry_1.default.getLocalMqttClient(emqxConfig, this.node);
                        ...
                        this.node.warn(`⚠️ MQTT/LOG ERROR: ${name} | ${mqttError.message}`);
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.info(this.node, 'Schedule sync completed successfully');
                // If sync is successful, update is_synced to 1
                await (0, outboxAck_1.markOutboxSynced)(this.scheduleRepo, 'tabiot_schedule', updated.name);
    ...
    async handleGetRunningSchedules(query, msg) {
        try {
    ...
    async handleDelete(path, msg) {
        ...
        try {
            ...
            const result = await this.scheduleRepo.update({ name }, {
                is_deleted: 1,
                is_from_local: 1,
                is_synced: 0,
                modified: new Date(),
            ...
            try {
                ...
                logger_1.logger.info(this.node, 'Schedule sync completed successfully');
                // If sync is successful, update is_synced to 1
                await (0, outboxAck_1.markOutboxSynced)(this.scheduleRepo, 'tabiot_schedule', updatedSchedule.name);
