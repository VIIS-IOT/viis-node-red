...
class ConfigService {
    ...
    /**
     * Check if configuration is valid (all sections)
     */
    isConfigValid() {
        try {
            return this.isFanConfigValid() && this.isWaterPumpConfigValid() && this.isCurtainConfigValid();
        }
        catch (error) {
    ...
    /**
     * Check if fan configuration is valid
     */
    isFanConfigValid() {
        try {
            ...
            if (!isValid) {
                this.logger.warn("Fan configuration validation failed");
            }
            return isValid;
        }
        catch (error) {
            this.logger.error(`Fan config validation error: ${error.message}`);
            return false;
        }
    }
    /**
     * Check if water pump configuration is valid
     */
    isWaterPumpConfigValid() {
        try {
            const config = this.getConfig();
            const checks = [
            ...
            const isValid = checks.every(check => check);
            if (!isValid) {
                this.logger.warn("Water pump configuration validation failed");
            }
            return isValid;
        }
        catch (error) {
            this.logger.error(`Water pump config validation error: ${error.message}`);
            return false;
        }
    }
    /**
     * Check if curtain configuration is valid
     */
    isCurtainConfigValid() {
        try {
            const config = this.getConfig();
            const checks = [
            ...
            const isValid = checks.every(check => check);
            if (!isValid) {
                this.logger.warn("Curtain configuration validation failed");
            }
            return isValid;
        }
        catch (error) {
            this.logger.error(`Curtain config validation error: ${error.message}`);
            return false;
        }
    }
    /**
