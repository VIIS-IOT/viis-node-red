...
describe('ViisFlowAccumulationNode', () => {
    ...
    beforeAll(async () => {
        ...
        dataSource = new typeorm_1.DataSource({
            ...
            entities: [TabiotFlowAccumulation_1.TabiotFlowAccumulation, TabiotDeviceTelemetry_1.TabiotDeviceTelemetry],
            synchronize: true,
            dropSchema: true,
            logging: false,
        });
    ...
    describe('Manual Calculation', () => {
        ...
        it('should be idempotent (can recalculate same hour)', async () => {
            ...
            const results2 = await accumulationService.calculateHourlyAccumulation(testDeviceId, hourStart);
            expect(results1[0].accumulated_tons).toBe(results2[0].accumulated_tons);
            // Verify only one record exists
            const accRepo = dataSource.getRepository(TabiotFlowAccumulation_1.TabiotFlowAccumulation);
    ...
    describe('Backfill', () => {
        it('should backfill multiple hours of data', async () => {
            ...
            const processedHours = await accumulationService.backfillAccumulation(testDeviceId, startDate, endDate);
    ...
    describe('Error Handling', () => {
        it('should handle database connection errors', async () => {
            const brokenDataSource = new typeorm_1.DataSource({
                type: 'mysql',
                host: 'invalid-host',
                port: 9999,
                username: 'invalid',
                password: 'invalid',
                database: 'invalid',
                entities: [TabiotFlowAccumulation_1.TabiotFlowAccumulation],
            });
            const brokenService = new FlowAccumulationService_1.FlowAccumulationService(brokenDataSource);
            await expect(brokenService.calculatePreviousHour('test-device')).rejects.toThrow();
        });
