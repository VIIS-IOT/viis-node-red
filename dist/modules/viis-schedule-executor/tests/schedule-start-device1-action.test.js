    ...
    Object.defineProperty(service, 'globalHelper', {
        value: {
            getJsonEnvVar: jest.fn((key, fallback) => {
                if (key === 'MODBUS_BOARD1_COILS' || key === 'MODBUS_COILS')
                    return DEVICE1_COILS;
...
test('customer-style valves produce valve_program = 3', () => {
    const service = createService();
    const mapped = service.mapScheduleToModbus(createSchedule(Object.assign(Object.assign({}, sampleAction), { valve_2: 'false' })));
    expect(mapped.holdingCommands[0]).toMatchObject({
...
test('START writes valve_program before power; set_ec retry does not skip power', async () => {
    ...
    const readHoldingRegisters = jest.fn(async (address) => ({
    ...
    const report = await service.executeModbusCommands(modbusClient, { holdingCommands: mapped.holdingCommands, coilCommands: mapped.coilCommands }, schedule);
    const firstReg20 = writeRegister.mock.calls.findIndex((c) => c[0] === 20 && c[1] === 7);
    ...
    expect(writeRegister.mock.calls.filter((c) => c[0] === 17)).toHaveLength(3);
