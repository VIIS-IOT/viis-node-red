...
describe('MQTT Retry Mechanism', () => {
    ...
    describe('publishMqttNotificationWithRetry', () => {
        test('should successfully publish on first attempt', async () => {
            ...
            await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true);
            expect(mockThingsboardClient.publish).toHaveBeenCalledTimes(1);
            expect(mockEmqxClient.publish).toHaveBeenCalledTimes(1);
            expect(mockNode.warn).toHaveBeenCalledWith(expect.stringContaining('📡 MQTT PUBLISHED'));
        });
        test('should retry ThingsBoard publish on failure', async () => {
            ...
            mockEmqxClient.publish.mockResolvedValue(true);
            const schedule = createTestSchedule();
            await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true, { maxRetries: 3, baseDelay: 100 });
        ...
        test('should continue if ThingsBoard fails but EMQX succeeds', async () => {
            ...
            await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true, { maxRetries: 2, baseDelay: 100 });
            expect(mockThingsboardClient.publish).toHaveBeenCalledTimes(2);
            expect(mockEmqxClient.publish).toHaveBeenCalledTimes(1);
            expect(mockNode.warn).toHaveBeenCalledWith(expect.stringContaining('⚠️ MQTT PARTIAL SUCCESS'));
        });
        ...
        test('should log error if both MQTT brokers fail after retries', async () => {
            ...
            const schedule = createTestSchedule();
            await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true, { maxRetries: 2, baseDelay: 100 });
            expect(mockNode.warn).toHaveBeenCalledWith(expect.stringContaining('❌ MQTT COMPLETE FAILURE'));
        });
        test('should use exponential backoff between retries', async () => {
            ...
            const schedule = createTestSchedule();
            try {
                await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true, { maxRetries: 3, baseDelay: 100, useExponentialBackoff: true });
            }
            catch (error) {
                // Expected to fail
            }
    ...
    describe('publishConfigUpdateWithRetry', () => {
        test('should retry config parameter publishing', async () => {
            ...
            await scheduleService.publishConfigUpdateWithRetry(mockThingsboardClient, mockEmqxClient, configParam, { maxRetries: 3, baseDelay: 100 });
            expect(mockThingsboardClient.publish).toHaveBeenCalledTimes(2);
        });
        test('should not throw error if config publish fails - degraded mode', async () => {
            ...
            await expect(scheduleService.publishConfigUpdateWithRetry(mockThingsboardClient, mockEmqxClient, configParam, { maxRetries: 2, baseDelay: 50 })).resolves.not.toThrow();
            expect(mockNode.warn).toHaveBeenCalledWith(expect.stringContaining('⚠️ CONFIG PUBLISH FAILED'));
        });
    });
    describe('MQTT Queue for Failed Messages', () => {
        test('should queue failed messages for later retry', async () => {
            ...
            const schedule = createTestSchedule();
            await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true, { maxRetries: 1, baseDelay: 50, queueOnFailure: true });
            // Check if message was queued
            const failedQueue = scheduleService.getFailedMqttQueue();
        ...
        test('should retry queued messages when connection restored', async () => {
            ...
            await scheduleService.processFailedMqttQueue(mockThingsboardClient, mockEmqxClient);
            // Verify queued message was sent
            const queue = scheduleService.getFailedMqttQueue();
        ...
        test('should limit queue size to prevent memory overflow', async () => {
            ...
            for (let i = 0; i < 150; i++) {
                ...
                await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true, { maxRetries: 1, baseDelay: 10, queueOnFailure: true });
            }
            const queue = scheduleService.getFailedMqttQueue();
    ...
    describe('MQTT Connection Health Check', () => {
        test('should check broker connection before publishing', async () => {
            ...
            const schedule = createTestSchedule();
            await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true, { maxRetries: 1, baseDelay: 50, checkConnection: true });
            // Should skip ThingsBoard since it's disconnected
            expect(mockNode.warn).toHaveBeenCalledWith(expect.stringContaining('⚠️ ThingsBoard disconnected'));
        });
        test('should attempt reconnection if both brokers disconnected', async () => {
            ...
            mockEmqxClient.publish.mockResolvedValue(true);
            const schedule = createTestSchedule();
            await scheduleService.publishMqttNotificationWithRetry(mockThingsboardClient, mockEmqxClient, schedule, true, { maxRetries: 1, baseDelay: 50, checkConnection: true, attemptReconnect: true });
