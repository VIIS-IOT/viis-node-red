...
function outcome(partial) {
    return Object.assign({ phase: 'set_holding', fc: 6, address: 1, expected: 1, attempts: 1 }, partial);
}
test('push after simulated work only; step order equals push order', () => {
    const buffer = new schedule_execution_buffer_1.ExecutionStepBuffer('run-1', 'start', 'sched-1');
    buffer.pushPhase({
        phase: 'set_holding',
...
test('failedOutcomes ignores pass, skipped, and blocked', () => {
    const buffer = new schedule_execution_buffer_1.ExecutionStepBuffer('run-2', 'start', 'sched-2');
    buffer.pushPhase({
        phase: 'set_holding',
        keys: [
    ...
    const fails = (0, schedule_execution_types_1.failedOutcomes)(buffer.toReport());
