...
const createTestSchedule = (overrides = {}) => (Object.assign({ name: 'test-schedule-001', action: JSON.stringify({ main_pump: "true" }), label: 'Test Schedule', device_label: 'Test Device', status: '', start_time: '08:00:00', end_time: '18:00:00', start_date: '2025-01-01', end_date: '2025-12-31', enable: 1, is_deleted: 0, device_id: 'test-device-001', created: new Date(), modified: new Date(), type: 'interval', deleted: null, interval: undefined }, overrides));
describe('ScheduleService - Interval (Day of Week) Validation', () => {
    ...
    beforeEach(() => {
        ...
        Object.defineProperty(scheduleService, 'globalHelper', {
            value: {
                getJsonEnvVar: jest.fn((key, defaultValue) => defaultValue),
                getEnvVar: jest.fn((key, defaultValue) => {
                    if (key === 'DEVICE_ID')
                        return 'test-device-001';
                    return defaultValue;
                }),
                getNumericEnvVar: jest.fn((key, defaultValue) => defaultValue)
            },
            writable: true,
            configurable: true
        });
    });
    const setSystemTime = (dateString) => {
    ...
    describe('Single Day Interval', () => {
        test('should allow schedule to run on specified single day (Wednesday = 3)', () => {
            ...
            expect(mockNode.warn).toHaveBeenCalledWith(expect.stringContaining('interval check passed: day 3 in [3]'));
        });
        test('should block schedule on different day (Thursday when interval is Wednesday)', () => {
            ...
            expect(mockNode.warn).toHaveBeenCalledWith(expect.stringContaining('skipped: current day 4 not in interval [3]'));
        });
    });
    ...
    describe('Invalid Interval Format', () => {
        test('should fallback to allowing schedule on invalid JSON format', () => {
            ...
            expect(scheduleService.isScheduleDue(schedule)).toBe(true);
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Error parsing interval'));
