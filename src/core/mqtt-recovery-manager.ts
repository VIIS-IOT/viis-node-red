...
interface RecoveryOptions {
    ...
    quickRecoveryInterval?: number;
    normalRecoveryInterval?: number;
    maxQuickRecoveryAttempts?: number;
    networkCheckInterval?: number;
    powerOutageDetection?: boolean;
    powerOutageThreshold?: number;
}
...
interface ClientInfo {
    ...
    inQuickRecovery: boolean;
}

export class MqttRecoveryManager extends EventEmitter {
    private static instance: MqttRecoveryManager;
    private clients: Map<string, ClientInfo> = new Map();
    private quickRecoveryTimer: NodeJS.Timeout | null = null;
    private normalRecoveryTimer: NodeJS.Timeout | null = null;
    private networkCheckTimer: NodeJS.Timeout | null = null;
    private consecutiveDisconnections = 0;
    private isPowerOutage = false;
    private isNetworkDisrupted = false;
    private lastNetworkCheckTime = Date.now();
    private networkRecoveryAttempts = 0;

    // Configuration with aggressive defaults
    private readonly QUICK_RECOVERY_INTERVAL: number;
    private readonly NORMAL_RECOVERY_INTERVAL: number;
    private readonly MAX_QUICK_RECOVERY_ATTEMPTS: number;
    private readonly NETWORK_CHECK_INTERVAL: number;
    private readonly AGGRESSIVE_MODE: boolean;
    private readonly POWER_OUTAGE_DETECTION: boolean;
    private readonly POWER_OUTAGE_THRESHOLD: number;

    // State tracking
    private isInPowerOutageRecovery = false;
    private lastNetworkState = true;
    ...
    private constructor(options: RecoveryOptions = {}) {
        ...
        this.NORMAL_RECOVERY_INTERVAL = options.normalRecoveryInterval ?? 10000; // 10 seconds
        this.MAX_QUICK_RECOVERY_ATTEMPTS = options.maxQuickRecoveryAttempts ?? 10;
        this.NETWORK_CHECK_INTERVAL = options.networkCheckInterval ?? 5000; // 5 seconds
        this.POWER_OUTAGE_DETECTION = options.powerOutageDetection ?? true;
        this.POWER_OUTAGE_THRESHOLD = options.powerOutageThreshold ?? 2;
    }

    public static getInstance(options?: RecoveryOptions): MqttRecoveryManager {
        if (!this.instance) {
     ...
     */
    private setupClientListeners(clientId: string, client: MqttClientCore, node: Node): void {
     ...
     */
    private startRecoveryMechanisms(): void {
     ...
     */
    private performNormalRecoveryCheck(): void {
        for (const [clientId, clientInfo] of this.clients.entries()) {
            if (!clientInfo.client.isConnected() && !clientInfo.inQuickRecovery) {
     ...
     */
    private attemptQuickRecovery(clientId: string): void {
        const clientInfo = this.clients.get(clientId);
        ...
        try {
            ...
            clientInfo.node.error(`[RECOVERY-MANAGER] Quick recovery failed for ${clientId}: ${(error as Error).message}`);
        }
    }
     ...
     */
    private attemptNormalRecovery(clientId: string): void {
        const clientInfo = this.clients.get(clientId);
        if (!clientInfo) return;
        ...
        try {
            ...
            clientInfo.node.error(`[RECOVERY-MANAGER] Normal recovery failed for ${clientId}: ${(error as Error).message}`);
        }
    }
    ...
    private checkNetworkConnectivity(): void {
        ...
        const allDisconnected = Array.from(this.clients.values()).every(
            client => !client.client.isConnected()
        );
     ...
     */
    private handlePowerOutage(): void {
        if (this.isInPowerOutageRecovery) return;
     ...
     */
    private handlePowerRestoration(): void {
        if (!this.isInPowerOutageRecovery) return;
     ...
     * Force immediate recovery on all clients
     */
    public forceImmediateRecovery(): void {
        Array.from(this.clients.keys()).forEach(clientId => {
            const clientInfo = this.clients.get(clientId);
     ...
     */
    private stopRecoveryMechanisms(): void {
        if (this.quickRecoveryTimer) {
