import {
    deviceProtocolKey,
    fromDeviceSchedule,
    fromDeviceSchedulePlan,
    toDeviceSchedule,
    toDeviceScheduleLog,
    toDeviceSchedulePlan,
    unwrapDevicePlanList,
} from '../demeter-schedule-protocol';
...
describe('demeter-schedule-protocol', () => {
    ...
    it('maps local schedule PK name to wire id and label to wire name', () => {
        const wire = toDeviceSchedule(localSchedule);
    ...
    it('keeps legacy Frappe payloads that only have name as the protocol key', () => {
        expect(deviceProtocolKey({ name: 'old-pk' })).toBe('old-pk');
        const local = fromDeviceSchedule({
            name: 'old-pk',
            label: 'Legacy label',
            enable: 1,
    ...
    it('maps local plan PK name to wire id and label to wire name', () => {
        const wire = toDeviceSchedulePlan(localPlan);
    ...
    it('maps Demeter wire plan back onto local name/label columns', () => {
        const local = fromDeviceSchedulePlan({
            id: 'rk-plan-1',
            name: 'Summer plan',
            enable: true,
        ...
        expect(local.schedules?.[0].name).toBe('rk-schedule-1');
        expect(local.schedules?.[0].label).toBe('Morning irrigation');
    });

    it('puts local schedule.name on schedule log as the device protocol key', () => {
        const body = toDeviceScheduleLog({ schedule_id: 'rk-schedule-1', start_time: 't0', end_time: 't1' });
        expect(body.schedule_id).toBe('rk-schedule-1');
    });

    it('unwraps both paginated and raw-array Demeter envelopes', () => {
        expect(unwrapDevicePlanList({ result: { data: [{ id: 'a', name: 'A' }] } })).toEqual([
            { id: 'a', name: 'A' },
        ]);
        expect(unwrapDevicePlanList({ result: [{ id: 'b', name: 'B' }] })).toEqual([
            { id: 'b', name: 'B' },
        ]);
        expect(unwrapDevicePlanList([{ id: 'c', name: 'C' }])).toEqual([{ id: 'c', name: 'C' }]);
    });
});
