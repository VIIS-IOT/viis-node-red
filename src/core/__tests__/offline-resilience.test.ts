...
describe('ExternalServiceCircuitBreaker', () => {
    let circuitBreaker: ExternalServiceCircuitBreaker;

    beforeEach(() => {
        circuitBreaker = ExternalServiceCircuitBreaker.getInstance();
...
describe('getExternalServicesStatus', () => {
    ...
    it('should return all online when circuits are closed', () => {
        const status = getExternalServicesStatus();
...
describe('Integration: Circuit Breaker with executeWithCircuitBreaker', () => {
    let circuitBreaker: ExternalServiceCircuitBreaker;
    const mockNode = { warn: jest.fn() } as any;
    ...
    it('should handle multiple failures and recovery', async () => {
        ...
        await executeWithCircuitBreaker('integration-test', mockNode, operation, 'fallback');
        await executeWithCircuitBreaker('integration-test', mockNode, operation, 'fallback');
        await executeWithCircuitBreaker('integration-test', mockNode, operation, 'fallback');
        ...
        // Call succeeds
        const result = await executeWithCircuitBreaker('integration-test', mockNode, operation, 'fallback');
        expect(result).toBe('success');
