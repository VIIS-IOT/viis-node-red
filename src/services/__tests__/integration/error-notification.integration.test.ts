...
import * as dataSourceModule from '../../../orm/dataSource';
...
describe('ErrorNotificationService - Real Database Integration', () => {
    ...
    let notificationRepo: Repository<TabiotNotification>;
    ...
    beforeAll(async () => {
        ...
        // Mock createDataSource to return our test dataSource
        (dataSourceModule.createDataSource as jest.Mock).mockResolvedValue(dataSource);
        ...
        const contextWithDataSource = {
            ...mockNodeContext,
            _getDataSource: () => Promise.resolve(dataSource)
        } as any;
    ...
    describe('🗄️ Database Operations', () => {
        ...
        it('should verify tabiot_notification table structure', async () => {
            const metadata = dataSource.getMetadata(TabiotNotification);
    ...
    describe('📝 Create Notification from Business Logic', () => {
        it('should create notification in real database', async () => {
            ...
            expect(found?.err_code).toBe('TEST_ERROR_001');
            expect(found?.message).toBe('Integration test error message');
            expect(found?.severity).toBe('high');
            expect(found?.entity).toBe('test_device_001');
    ...
    describe('🔄 Auto-Resolve with Real Database', () => {
        it('should auto-resolve notification when error clears (holding register)', async () => {
            ...
            const modbusError: ModbusErrorSource = {
                register_type: 'holding',
                address: 1000,
            ...
            console.log(`✅ Created error notification: ${notification?.name}`);

            // Clear error (value = 0)
            await errorNotificationService.autoResolveIfClearByBoardId(
            ...
            expect(resolved?.is_read).toBe(1);
        ...
        it('should auto-resolve coil errors', async () => {
            ...
            // Create coil error
            const notification = await errorNotificationService.createFromModbusByBoardId(
                { register_type: 'coil', address: 500, value: true },
                'board1',
                entity
            );

            expect(notification).not.toBeNull();
            console.log(`✅ Created coil error: ${notification?.name}`);

            // Clear coil (value = false)
            await errorNotificationService.autoResolveIfClearByBoardId(
                { register_type: 'coil', address: 500, value: false },
                'board1',
                entity
            );
        ...
        it('should NOT auto-resolve if auto_resolve is false', async () => {
            ...
            const notification = await errorNotificationService.createFromModbusByBoardId(
                { register_type: 'holding', address: 1000, value: 2 }, // ERR_TEMP_SENSOR_FAULT
                'board1',
                entity
            );

            expect(notification).not.toBeNull();
            expect(notification?.err_code).toBe('ERR_TEMP_SENSOR_FAULT');
            ...
            expect(stillUnresolved?.is_read).toBe(0);
    ...
    describe('🎯 Error Mapping from Global Context', () => {
        it('should parse error from global context mappings', () => {
            ...
            const parsedError = errorMappingService.parseModbusErrorByBoardId(
                source,
                'board1'
            );

            expect(parsedError).not.toBeNull();
            expect(parsedError?.err_code).toBe('ERR_TEMP_HIGH');
            expect(parsedError?.message).toBe('Nhiệt độ vượt ngưỡng an toàn');
            expect(parsedError?.severity).toBe('high');
            expect(parsedError?.auto_resolve).toBe(true);
    ...
    describe('🔍 Edge Cases with Real Database', () => {
        ...
        it('should handle concurrent duplicate detections (known race condition)', async () => {
            ...
            const promises = Array(5).fill(null).map(() => 
                errorNotificationService.createFromBusinessLogic(errorData)
            );
