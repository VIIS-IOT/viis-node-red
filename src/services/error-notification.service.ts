...
import { ErrorMappingService, ModbusErrorSource, ParsedError } from './error-mapping.service';
...
export class ErrorNotificationService {
  ...
  async createFromBusinessLogic(errorData: BusinessLogicError): Promise<TabiotNotification> {
    ...
    return await this.createOrUpdateNotification(
      ...
      errorData.type,
      errorData.entity,
      errorData.entity_label || errorData.entity,
      errorData.metadata
    );
  }
  ...
  private async createOrUpdateNotification(
    ...
    if (existing) {
      ...
      updates.metadata = JSON.stringify({
        ...existingMetadata,
        occurrence_count: (existingMetadata.occurrence_count || 1) + 1,
        last_occurred: new Date().toISOString(),
        ...metadata
      });
   ...
   */
  private async findUnresolvedNotification(
    err_code: string,
    entity: string
  ): Promise<TabiotNotification | null> {
    if (!this.notificationRepo) {
   ...
   */
  private isErrorCleared(source: ModbusErrorSource): boolean {
   ...
   */
  private generateNotificationName(entity: string, err_code: string): string {
   ...
   */
  private severityToType(severity: string): 'info' | 'warning' | 'error' | 'alert' {
    switch (severity) {
      case 'low':
        return 'info';
      case 'medium':
   ...
   */
  async createFromModbusByBoardId(
    source: ModbusErrorSource,
    boardId: string,
    entity: string
  ): Promise<TabiotNotification | null> {
    ...
    const businessLogicError: BusinessLogicError = {
   ...
   * @returns True if notification was resolved
   */
  async autoResolveIfClearByBoardId(
    source: ModbusErrorSource,
    boardId: string,
    entity: string
  ): Promise<boolean> {
