/**
 * @fileoverview Sync State Service for VIIS Sync Production Function module
 * Tracks and manages the state of production function synchronization operations
 */
...
export interface SyncResult {
    ...
    success: boolean;
    /** Number of production functions created */
    functionsCreated?: number;
    /** Number of production functions updated */
    functionsUpdated?: number;
    /** Total number of production functions processed */
...
export class SyncStateService {
    ...
    getStatusMessage(): string {
        ...
        if (this.lastSyncResult && this.lastSyncResult.success) {
            return `Last sync: ${timeMessage} (${this.lastSyncResult.totalFunctions || 0} functions)`;
        } else {
