...
export class ModbusService implements IModbusService {
    ...
    /**
     * Get Modbus client for board2 specifically
     */
    async getBoard2Client(): Promise<any> {
        return await this.getModbusClient('board2');
    }

    /**
     * Find Modbus mapping for a specific board
     */
    findModbusMappingForBoard(key: string, boardId: string): ModbusMappingResult | null {
        const boardIdUpper = boardId.toUpperCase();

        // Check board-specific mappings
        const boardCoils = this.globalHelper.getJsonEnvVar(`MODBUS_${boardIdUpper}_COILS`, {});
        const boardHolding = this.globalHelper.getJsonEnvVar(`MODBUS_${boardIdUpper}_HOLDING_REGISTERS`, {});
        const boardInput = this.globalHelper.getJsonEnvVar(`MODBUS_${boardIdUpper}_INPUT_REGISTERS`, {});

        // Check holding registers
        if (boardHolding[key] !== undefined) {
            return {
                address: boardHolding[key],
                fc: MODBUS_FUNCTION_CODES.WRITE_SINGLE_REGISTER,
                value: 0,
                boardId: boardId
            };
    ...
    /**
     * Write to a specific board
     */
    async writeToModbusBoard(key: string, mapping: ModbusMappingResult, value: number | boolean, boardId: string): Promise<void> {
        try {
            let writeValue = value;
            ...
            // Get board-specific client
            const modbusClient = await this.getModbusClient(boardId);

            if (!modbusClient) {
                throw new Error(`Modbus client for board ${boardId} is not initialized`);
            }

            if (!modbusClient.isConnected) {
                throw new Error(`Modbus client for board ${boardId} not connected`);
            }
    ...
    /**
     * Read from a specific board
     */
    async readFromModbusBoard(key: string, mapping: ModbusMappingResult, boardId: string): Promise<number | boolean> {
        try {
            // Get board-specific client
            const modbusClient = await this.getModbusClient(boardId);
