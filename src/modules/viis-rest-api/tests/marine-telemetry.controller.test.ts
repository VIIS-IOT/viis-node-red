...
import { MachineType, MachineStatus } from '../dto/marine-telemetry.dto';
...
describe('MarineTelemetryController', () => {
    let controller: MarineTelemetryController;
    const TEST_DEVICE_ID = 'test_ship_001';

    beforeAll(() => {
        controller = new MarineTelemetryController(
            mockMarineTelemetryService as any,
            mockNode
        );
    });
    ...
    describe('GET /api/v2/marine/telemetry/latest/:device_id', () => {
        ...
        it('should calculate consumption rate correctly for Main Engine', async () => {
            ...
            const mainEngine = result.machines[MachineType.MAIN_ENGINE];
        ...
        it('should calculate consumption rate correctly for Boiler', async () => {
            ...
            const boiler = result.machines[MachineType.BOILER];
        ...
        it('should filter by specified sensor keys', async () => {
            ...
            expect(mockMarineTelemetryService.getLatestTelemetry).toHaveBeenCalledWith(
                TEST_DEVICE_ID,
                ['fs01', 'fs02']
            );
    ...
    describe('GET /api/v2/marine/telemetry/machines/:device_id', () => {
        ...
        it('should show OPERATIONAL status when recent data exists', async () => {
            ...
            result.machines.forEach(machine => {
                expect(machine.status).toBe(MachineStatus.OPERATIONAL);
        ...
        it('should show NO_DATA status when no telemetry', async () => {
            const noDataResponse = {
                ...mockMachinesSummaryResponse,
                machines: mockMachinesSummaryResponse.machines.map(m => ({
                    ...m,
            ...
            result.machines.forEach(machine => {
                expect(machine.status).toBe(MachineStatus.NO_DATA);
