...
import { TabiotSchedule, ModbusCmd, ConfigParameter } from '../../common/types';
...
jest.mock('../../../../ultils/global-context-helper', () => ({
    GlobalContextHelper: jest.fn().mockImplementation((context: any) => ({
        getEnvVar: jest.fn((key: string, defaultValue: any) => {
            if (key === 'MODBUS_BOARDS') return null;
            return defaultValue;
        }),
...
describe('ScheduleMapperService', () => {
    ...
    beforeEach(() => {
        ...
        service = new ScheduleMapperService(mockNode, true);
    });

    describe('Constructor', () => {
        it('should initialize with debug enabled', () => {
            const debugService = new ScheduleMapperService(mockNode, true);
            expect(debugService).toBeDefined();
        });

        it('should initialize with debug disabled', () => {
            const nonDebugService = new ScheduleMapperService(mockNode, false);
    ...
    describe('loadAllModbusCoils', () => {
        ...
        it('should load multi-board coils when configured', () => {
            const mockService = new ScheduleMapperService(mockNode, false);
            (mockService as any).globalHelper.getEnvVar = jest.fn().mockReturnValue(
                JSON.stringify([
                    { id: 'board1', name: 'Board 1' },
                    { id: 'board2', name: 'Board 2' }
                ])
            );
            (mockService as any).globalHelper.getJsonEnvVar = jest.fn((key: string) => {
                if (key === 'MODBUS_BOARD1_COILS') return { 'valve_1': 10 };
                if (key === 'MODBUS_BOARD2_COILS') return { 'valve_2': 20 };
                return {};
            });
        ...
        it('should handle invalid MODBUS_BOARDS gracefully', () => {
            const mockService = new ScheduleMapperService(mockNode, false);
            (mockService as any).globalHelper.getEnvVar = jest.fn().mockReturnValue('invalid json');
    ...
    describe('loadAllModbusHoldingRegisters', () => {
        ...
        it('should load multi-board holding registers', () => {
            ...
            (mockService as any).globalHelper.getJsonEnvVar = jest.fn((key: string) => {
                if (key === 'MODBUS_BOARD1_HOLDING_REGISTERS') return { 'iri_time': 0, 'test': 50 };
                return {};
            });
    ...
    describe('storeConfigParameter', () => {
        ...
        it('should store numeric config parameter', () => {
            ...
            expect(result?.key).toBe('custom_param');
        ...
        it('should store string config parameter', () => {
            ...
            expect(result?.value).toBe('Test Label');
        });
    ...
    describe('mapScheduleToModbus', () => {
        ...
        it('should map schedule action to holding commands', () => {
            ...
            expect(setFlowCmd?.value).toBe(50);
            expect(setFlowCmd?.fc).toBe(6);
        });
        ...
        it('should map schedule action to coil commands', () => {
            ...
            expect(valve1Cmd?.value).toBe(true);
            expect(valve1Cmd?.fc).toBe(5);
        });
    ...
    describe('Luoi mapping', () => {
        ...
        it('should skip luoi_1=2 (N/A) — no coil commands generated', () => {
            ...
            expect(luoi2Dai?.value).toBe(true);
        });
