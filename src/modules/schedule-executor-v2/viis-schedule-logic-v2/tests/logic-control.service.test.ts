...
import { ControlMode, SafetyConditions, ModbusCmd } from '../../common/types';
...
const createMockNode = () => {
    const contextObject = {
        global: {
            get: jest.fn((key: string) => mockGlobalContext.get(key)),
...
describe('LogicControlService', () => {
    let service: LogicControlService;
    let mockNode: Node;
    ...
    beforeEach(() => {
        ...
        service = new LogicControlService(mockNode, true);
    });

    describe('Constructor', () => {
        it('should initialize with debug enabled', () => {
            const debugService = new LogicControlService(mockNode, true);
            expect(debugService).toBeDefined();
        });

        it('should initialize with debug disabled', () => {
            const nonDebugService = new LogicControlService(mockNode, false);
    ...
    describe('readControlMode', () => {
        ...
        it('should return OFF when control mode is 0', async () => {
            ...
            const mockService = new LogicControlService(mockNode, false);
        ...
        it('should default to AUTO on error', async () => {
            ...
            const mockService = new LogicControlService(mockNode, false);
            (mockService as any).globalHelper.getJsonEnvVar = jest.fn().mockImplementation(() => {
    ...
    describe('hasCommandOverlap', () => {
        it('should detect overlapping commands', () => {
            const activeCommands = {
                'schedule-1': [
                    { key: 'valve_1', value: true, fc: 5, unitid: 1, address: 10, quantity: 1 } as ModbusCmd
                ]
            };

            const newCommands = [
                { key: 'valve_1', value: false, fc: 5, unitid: 1, address: 10, quantity: 1 } as ModbusCmd
            ];
        ...
        it('should not detect overlap when commands are different', () => {
            ...
            const newCommands = [
                { key: 'valve_2', value: true, fc: 5, unitid: 1, address: 11, quantity: 1 } as ModbusCmd
            ];
    ...
    describe('handleRpcSetMode', () => {
        it('should set control mode to AUTO', async () => {
            ...
            expect(mockNode.context().global.set).toHaveBeenCalledWith(
                'CONTROL_MODE',
                1  // Numeric value for AUTO
            );
        });
    ...
    describe('clearManualOverrides', () => {
        ...
        it('should clear specific override', () => {
            ...
            const setCalls = (mockNode.context().global.set as jest.Mock).mock.calls;
