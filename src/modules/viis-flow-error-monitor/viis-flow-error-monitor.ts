...
interface FlowErrorMonitorConfig extends NodeDef {
    name: string;
    toleranceRunning: number;    // Tolerance when machine is running
...
module.exports = function (RED: NodeAPI) {
    function ViisFlowErrorMonitorNode(this: Node, config: FlowErrorMonitorConfig) {
        ...
        node.on('input', async (msg: any, send: any, done: any) => {
            ...
            try {
                ...
                const timeValveData: ChannelData = {
                ...
                function shouldLogError(lastTime: number, currentTime: number): boolean {
                    const elapsedMinutes = (currentTime - lastTime) / 60000;
                    return elapsedMinutes >= debounceMinutes;
                }

                async function createBusinessError(
                    errCode: string,
                    msgText: string,
                ...
                async function checkFlowWithToleranceWhenFinish(tolerance: number) {
                    const errors: any[] = [];
