...
export class ViisTelemetryProcessor {
  ...
  async processTelemetryData(event: TelemetryDataEvent): Promise<void> {
    ...
    const savedKeyCountKey = `${CONTEXT_KEYS.THRESHOLD_CONFIG}_count_${this.node.id}`;
    const savedKeyCount = (this.flowContext.get(savedKeyCountKey) as number | undefined) ?? -1;
    ...
    if (configChanged) {
      ...
      this.resetState();
      const freshChangedKeys = getChangedKeys(currentState, {}, thresholdConfig);
      await this.publishTelemetryData(freshChangedKeys);
      this.setLastSent(now);
      this.logDebug(
        `[${source}] Published full snapshot after config change (${savedKeyCount} → ${currentKeyCount} keys)`
      );
    }
