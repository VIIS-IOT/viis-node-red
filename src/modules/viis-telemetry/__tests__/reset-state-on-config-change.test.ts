...
import { ViisTelemetryProcessor } from '../viis-telemetry-processor';
import { CONTEXT_KEYS } from '../viis-telemetry-constants';
import type { ThresholdConfig } from '../viis-telemetry-utils';
...
import { publishTelemetry } from '../viis-telemetry-utils';

const NODE_ID = 'test-node-pmr005';

interface MockStores {
  flowStore: Record<string, any>;
  nodeStore: Record<string, any>;
  flowContext: any;
  nodeContext: any;
  node: any;
  localMqtt: any;
  tbMqtt: any;
  processor: ViisTelemetryProcessor;
}

function buildProcessor(): MockStores {
  ...
  const flowContext = {
    get: jest.fn((key: string) => flowStore[key]),
  ...
  const nodeContext = {
    get: jest.fn((key: string) => nodeStore[key]),
    set: jest.fn((key: string, value: any) => {
      nodeStore[key] = value;
    }),
    global: {
  ...
  const processor = new ViisTelemetryProcessor(
    node,
    nodeContext as any,
    flowContext as any,
    localMqtt as any,
...
function setConfig(store: MockStores, cfg: ThresholdConfig): void {
  store.flowStore[CONTEXT_KEYS.THRESHOLD_CONFIG] = cfg;
}

function savedKeyCount(store: MockStores): number | undefined {
  return store.flowStore[`${CONTEXT_KEYS.THRESHOLD_CONFIG}_count_${NODE_ID}`];
}
...
describe('PMR-005: reset state on thresholdConfig change', () => {
  it('Test 1: first poll with 3-key config records baseline; no resetState, normal publish', async () => {
    ...
    const cfg: ThresholdConfig = { temp: 1, hum: 1, press: 1 };
    setConfig(store, cfg);
    ...
    expect(store.nodeStore[CONTEXT_KEYS.PREVIOUS_STATE]).toEqual({
      temp: 20,
      hum: 50,
      press: 1010,
    });
  ...
  it('Test 2: updating to a different-shape config (3 → 5 keys) triggers resetState and full snapshot', async () => {
    ...
    expect(savedKeyCount(store)).toBe(3);
    const publishCountAfterSeed = (publishTelemetry as jest.Mock).mock.calls.length;

    // Update config via input-handler path.
    const newCfg: ThresholdConfig = { temp: 1, hum: 1, press: 1, flow: 1, level: 1 };
    store.processor.updateThresholdConfig(newCfg);

    // Next poll — must detect configChanged (3 → 5) and reset state.
    (publishTelemetry as jest.Mock).mockClear();
    await store.processor.processTelemetryData({
    ...
    const publishCall = (publishTelemetry as jest.Mock).mock.calls[0][0];
