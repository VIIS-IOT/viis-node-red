...
import { normalizeValue, normalizeTelemetryData } from '../viis-telemetry-utils';

describe('normalizeValue', () => {
  describe('null and undefined handling', () => {
    it('should return null for null input', () => {
      expect(normalizeValue('any_key', null)).toBeNull();
    });

    it('should return null for undefined input', () => {
      expect(normalizeValue('any_key', undefined)).toBeNull();
    });
  });

  describe('string keys preservation', () => {
    it('should keep oil_profile as string', () => {
      expect(normalizeValue('fs01_oil_profile', 'HFO_Generator')).toBe('HFO_Generator');
    });

    it('should keep trip_id as string', () => {
      expect(normalizeValue('trip_id', '12345')).toBe('12345');
    });

    it('should keep hour_start as string', () => {
      expect(normalizeValue('hour_start', '2025-01-20T14:00:00Z')).toBe('2025-01-20T14:00:00Z');
    });

    it('should keep gps_time as string', () => {
      expect(normalizeValue('gps_time', '123456 010125')).toBe('123456 010125');
    });
  ...
  describe('numeric value normalization', () => {
    it('should keep number as number', () => {
      expect(normalizeValue('fs01_flow', 25.5)).toBe(25.5);
    });
    ...
    it('should handle integer strings', () => {
      expect(normalizeValue('sample_count', '100')).toBe(100);
    });

    it('should handle negative numbers', () => {
      expect(normalizeValue('temperature', '-10.5')).toBe(-10.5);
    });

    it('should handle scientific notation', () => {
      expect(normalizeValue('small_value', '1.5e-3')).toBe(0.0015);
    });

    it('should return null for NaN', () => {
      expect(normalizeValue('bad_value', NaN)).toBeNull();
    });

    it('should return null for Infinity', () => {
      expect(normalizeValue('bad_value', Infinity)).toBeNull();
    });
    ...
    it('should return null for empty string', () => {
      expect(normalizeValue('empty', '')).toBeNull();
    });
  ...
  describe('boolean handling', () => {
    it('should keep true as true', () => {
      expect(normalizeValue('is_active', true)).toBe(true);
    });

    it('should keep false as false', () => {
      expect(normalizeValue('is_active', false)).toBe(false);
    });
  });

  describe('non-numeric string handling', () => {
    it('should keep non-numeric strings as strings', () => {
      expect(normalizeValue('status', 'running')).toBe('running');
    });

    it('should trim whitespace', () => {
      expect(normalizeValue('status', '  active  ')).toBe('active');
    });
  });
});
...
describe('normalizeTelemetryData', () => {
  it('should normalize all values in an object', () => {
    ...
    const result = normalizeTelemetryData(input);
