...
import { GLOBAL_CONTEXT_KEYS } from '../viis-telemetry-constants';
import { ScaleConfig } from '../viis-telemetry-utils';
...
describe('ViisTelemetryPollingService - Flow Sensor Division', () => {
  ...
  let globalContextStore: any;

  // Default scale configs for fs01-fs06 (divide by 10)
  const flowSensorScaleConfigs: ScaleConfig[] = [
  ...
  beforeEach(() => {
    ...
    globalContextStore[GLOBAL_CONTEXT_KEYS.SCALE_CONFIGS] = flowSensorScaleConfigs;
  ...
  describe('Holding Registers - Flow Sensor Keys (fs01-fs06)', () => {
    it('should divide fs01 value by 10 via ScaleConfig when reading holding registers', async () => {
      ...
      const processRegisterData = (pollingService as any).processRegisterData.bind(pollingService);
  ...
  describe('Integration with Scaling', () => {
    it('should apply custom scaling (multiply) instead of default division when configured', async () => {
      ...
      globalContextStore[GLOBAL_CONTEXT_KEYS.SCALE_CONFIGS] = [
