...
describe("viis-flow-calibration - Board2 Global Context Reading", () => {
    describe("Board2 Register Keys", () => {
        it("should generate correct key names for all 16 pumps", () => {
            // Test K_FACTOR keys
            expect(BOARD2_KEYS.K_FACTOR(1)).toBe("HOLDING_K_FACTOR_BOM_1");
            expect(BOARD2_KEYS.K_FACTOR(16)).toBe("HOLDING_K_FACTOR_BOM_16");

            // Test FLOWRATE keys
            expect(BOARD2_KEYS.FLOWRATE(1)).toBe("HOLDING_FLOWRATE_BOM_1");
            expect(BOARD2_KEYS.FLOWRATE(16)).toBe("HOLDING_FLOWRATE_BOM_16");

            // Test INPUT_TOTAL_FLOW keys
            expect(BOARD2_KEYS.INPUT_TOTAL_FLOW(1)).toBe("INPUT_TOTAL_FLOW_BOM_1");
            expect(BOARD2_KEYS.INPUT_TOTAL_FLOW(16)).toBe("INPUT_TOTAL_FLOW_BOM_16");
        });
    });
    ...
    describe("Board2 Register Mapping from device1.json", () => {
        ...
        it("should have correct K-Factor addresses for all pumps", () => {
            ...
            for (let i = 1; i <= 16; i++) {
                const key = BOARD2_KEYS.K_FACTOR(i);
        ...
        it("should have correct Flowrate addresses for all pumps", () => {
            ...
            for (let i = 1; i <= 16; i++) {
                const key = BOARD2_KEYS.FLOWRATE(i);
                expect(board2.holdingRegisters[key]).toBeDefined();
        ...
        it("should have correct Input Total Flow addresses for all pumps", () => {
            ...
            for (let i = 1; i <= 16; i++) {
                const key = BOARD2_KEYS.INPUT_TOTAL_FLOW(i);
    ...
    describe("Board2 Register Value Lookup Simulation", () => {
        ...
        it("should correctly lookup K-Factor values for all pumps", () => {
            for (let i = 1; i <= 16; i++) {
                const key = BOARD2_KEYS.K_FACTOR(i);
                const value = mockBoard2HoldingRegisters[key as keyof typeof mockBoard2HoldingRegisters];
        ...
        it("should correctly lookup Input Total Flow values for all pumps", () => {
            for (let i = 1; i <= 16; i++) {
                const key = BOARD2_KEYS.INPUT_TOTAL_FLOW(i);
                const value = mockBoard2InputRegisters[key as keyof typeof mockBoard2InputRegisters];
                expect(value).toBeDefined();
        ...
        it("should handle missing keys gracefully with default fallback", () => {
            ...
            for (let i = 1; i <= 16; i++) {
                const key = BOARD2_KEYS.K_FACTOR(i);
                const value = (emptyRegisters as any)[key];
        ...
        it("should NOT fallback to default when value is 0", () => {
            ...
            const key = BOARD2_KEYS.K_FACTOR(1);
            const value = registersWithZero[key as keyof typeof registersWithZero];
    ...
    describe("Global Context Structure Validation", () => {
        it("should handle nested modbusMappings structure (for ADDRESSES)", () => {
            ...
            const board2HoldingRegisters = mockGlobalContext.modbusMappings.board2?.holdingRegisters || {};
        ...
        it("should handle flat structure fallback (for ADDRESSES)", () => {
            ...
            const board2HoldingRegisters = JSON.parse(mockGlobalContext.MODBUS_BOARD2_HOLDING_REGISTERS as string);
        ...
        it("should handle empty/undefined global context", () => {
            ...
            const board2HoldingRegisters = modbusMappings.board2?.holdingRegisters || {};
    ...
    describe("CalibrationService with Board2 Values", () => {
        const service = new CalibrationService("test-node", false);
