...
describe('DH6400 Processor Data Processing', () => {
     ...
     */
    function createMockDH6400Data(channels: number[]): Map<number, DH6400FlowData> {
        const mockData = new Map<number, DH6400FlowData>();
     ...
     */
    function processDH6400Data(dh6400Data: Map<number, DH6400FlowData>) {
        const tfsSensorData = [];
     ...
     */
    function processDH6400FlowData(dh6400Data: Map<number, DH6400FlowData>) {
    ...
    test('should process flow data for all 6 channels', () => {
        ...
        expect(fs01?.float_value).toBe(31.0);

        // Check fs06
        const fs06 = flowData.find(d => d.key_name === 'fs06');
        expect(fs06).toBeDefined();
        expect(fs06?.float_value).toBe(36.0);
    });
...
describe('DH6400 vs Modbus Data Comparison', () => {
     ...
     */
    function parseModbusTfs(registers: number[]): Map<string, number> {
        const tfsData = new Map<string, number>();
    ...
    test('should produce equivalent results to Modbus parsing', () => {
        ...
        const dh6400Data = new Map<number, DH6400FlowData>();
        dh6400Data.set(1, {
            channel: 1,
            slaveId: 1,
            sensorKey: 'fs01',
        ...
        // Process DH6400
        const dh6400Result = new Map<string, number>();
        for (const [channel, flowData] of dh6400Data) {
            const sensorKey = `tfs${String(channel).padStart(2, '0')}`;
 ...
 */
export function createMockDH6400Data(channels: number[]): Map<number, DH6400FlowData> {
    const mockData = new Map<number, DH6400FlowData>();
