...
module.exports = function (RED: NodeAPI) {
  ...
  function ViisAisTelemetryNode(this: Node, config: ViisAisTelemetryNodeDef) {
    ...
    let telemetryRepo: any = null;
    let dbInitStarted = false;
    let dbInitFailed = false;
    let dbInitAttempt = 0;
    let dbInitNextRetryAt = 0;
    let isSavingToDb = false;
    let pendingPayloadToSave: AisTelemetryPayload | null = null;
    let lastSavedTimestamp = 0;

    node.log(`[AIS] Initializing - Host: ${aisHost}:${aisPort}, Output interval: ${outputInterval}ms`);

    async function ensureDbInitialized(): Promise<void> {
      if (dbInitFailed || dbInitStarted) return;
      const now = Date.now();
      if (dbInitNextRetryAt && now < dbInitNextRetryAt) return;
      dbInitStarted = true;
      try {
        ...
        telemetryRepo = dataSource.getRepository(TabiotDeviceTelemetry);
        node.log('[AIS] Database initialized for local telemetry storage');
      } catch (err) {
        dbInitAttempt++;
        dbInitStarted = false;
        if (dbInitAttempt >= 10) {
        ...
        const delayMs = Math.min(60000, 1000 * Math.pow(2, Math.min(dbInitAttempt, 6)));
        dbInitNextRetryAt = Date.now() + delayMs;
        node.warn(`[AIS] Failed to initialize database for telemetry storage: ${(err as Error).message}`);
        return;
      } finally {
        dbInitStarted = false;
      }
    ...
    function getMonotonicTimestamp(): number {
      ...
      if (now <= lastSavedTimestamp) {
        lastSavedTimestamp = lastSavedTimestamp + 1;
    ...
    function sanitizeTelemetryKey(key: string): string {
      return key.replace(/[^a-zA-Z0-9_]/g, "_");
    }

    function buildTelemetryEntities(timestamp: number, payload: AisTelemetryPayload): TabiotDeviceTelemetry[] {
      ...
      const addScalar = (key: string, value: number | string | boolean) => {
        ...
        entity.key_name = `ais_${sanitizeTelemetryKey(key)}`;
    ...
    async function savePayloadToDatabase(payload: AisTelemetryPayload): Promise<void> {
      if (isClosing || dbInitFailed) return;
      if (isSavingToDb) {
      ...
      const entities = buildTelemetryEntities(timestamp, payload);
      if (entities.length === 0) return;

      isSavingToDb = true;
      try {
        await telemetryRepo.save(entities);
        logDebug(`[AIS] Saved ${entities.length} telemetry records to database`);
      } catch (err) {
        node.warn(`[AIS] Failed to save AIS telemetry to database: ${(err as Error).message}`);
      } finally {
        isSavingToDb = false;
        const nextPayload = pendingPayloadToSave;
        pendingPayloadToSave = null;
        if (nextPayload) {
          void savePayloadToDatabase(nextPayload);
        }
      }
    }
    ...
    async function outputData(): Promise<void> {
      ...
      try {
        ...
        void savePayloadToDatabase(payload);
