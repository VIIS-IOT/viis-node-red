...
describe('Fertilizer EC Control - State Machine', () => {
    ...
    describe('State Guards', () => {
        it('should prevent multiple starts when not in IDLE', () => {
            ...
            const canStart: boolean = (state as string) === 'IDLE' || (state as string) === 'ERROR';
...
type State = 'IDLE' | 'STARTING' | 'RAMPING_UP' | 'RUNNING' | 'STOPPING' | 'ERROR';
...
function canStartIrrigation(state: string): boolean {
    return state === 'IDLE' || state === 'ERROR';
}

function canStopIrrigation(state: string): boolean {
