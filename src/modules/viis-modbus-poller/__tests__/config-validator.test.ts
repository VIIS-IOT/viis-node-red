...
describe("validateResolvedConfig", () => {
  ...
  it("fails on missing polling config and non-positive intervals", () => {
    ...
    ).toContain("pollingConfig must define at least one poll group");
    ...
    ).toContain("realtime.interval must be a positive number");
  });
  ...
  it("fails on invalid scale config objects", () => {
    const result = validateResolvedConfig({
      ...
      scaleConfigs: [
        ...
        { key: "", operation: "multiply", factor: 0, direction: "sideways" } as any,
      ],
    });
