...
import type { ResolvedPollerConfig } from "../types";

function makeConfig(overrides: Partial<ResolvedPollerConfig> = {}): ResolvedPollerConfig {
  return {
    deviceId: "device-1",
    boardId: "board1",
    pollingConfig: { realtime: { interval: 2000, coils: ["power", "aux_pump"] } },
...
describe("multi-board poll helpers", () => {
  ...
  it("merges telemetry and keeps the other board when one read fails", () => {
    const merged = mergePollTickResults([
      {
