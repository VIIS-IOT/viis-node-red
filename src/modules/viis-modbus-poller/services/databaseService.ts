...
export class DatabaseService {
    ...
    async initialize(): Promise<void> {
        if (!this.initialized) {
            try {
                ...
                this.logger.errorWithStack('Failed to initialize database', error as Error);
                throw error;
            }
        } else {
    ...
    async destroy(): Promise<void> {
        if (this.initialized) {
            try {
                ...
                this.logger.errorWithStack('Error while destroying database connection', error as Error);
                throw error;
            }
        } else {
     ...
     */
    getTelemetryRepository(): Repository<TabiotDeviceTelemetry> {
     ...
     * @throws Error if database is not initialized
     */
    getTelemetryLatestRepository(): Repository<TabiotDeviceTelemetryLatest> {
        if (!this.isInitialized()) {
            const errorMessage = 'Database not initialized';
            this.logger.error(errorMessage);
            throw new Error(errorMessage);
    ...
    async isHealthy(): Promise<boolean> {
        try {
            ...
            this.logger.errorWithStack('Database health check failed', error as Error);
            return false;
        }
    }
     ...
     */
    getConnectionInfo(): { isInitialized: boolean; isConnected: boolean; driver: string } {
        return {
