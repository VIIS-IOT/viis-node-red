...
export class SensorService implements ISensorService {
    ...
    private cachedSensorData: SensorData | null = null;
    ...
    getSensorData(): SensorData | null {
        try {
            ...
            this.logger.error(`${ERROR_MESSAGES.SENSOR_DATA_ERROR}: ${(error as Error).message}`);
            return null;
        }
    }
    ...
    getDeviceStatus(): DeviceStatus | null {
        try {
            ...
            this.logger.error(`Failed to read device status: ${(error as Error).message}`);
            return null;
        }
    }
    ...
    isDataValid(): boolean {
        ...
        const hasCriticalData = (
            typeof sensorData.temp_indoor === 'number' &&
     ...
     */
    private validateNumericValue(value: any, fieldName: string): number | undefined {
     ...
     */
    private validateBooleanValue(value: any, fieldName: string): boolean | undefined {
        if (value === null || value === undefined) {
            return undefined;
        }
     ...
     */
    private isSensorCacheValid(): boolean {
        const now = Date.now();
     ...
     */
    private getActiveFanCount(deviceStatus: DeviceStatus): number {
        let count = 0;
        ['quat_1', 'quat_2', 'quat_3', 'quat_4', 'quat_5', 'quat_6'].forEach(fanKey => {
            if (deviceStatus[fanKey as keyof DeviceStatus] === true) {
                count++;
            }
        });
        return count;
