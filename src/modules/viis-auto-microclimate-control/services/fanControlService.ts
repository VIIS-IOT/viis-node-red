...
export class FanControlService implements IFanControlService {
     ...
     */
    private async processFanDaoSynchronization(timeOnMs: number, timeOffMs: number): Promise<ControlAction[]> {
        const stateKey = `${CONTEXT_KEYS.FAN_ROTATION_STATE}_dao_sync`;
        ...
        if (elapsed >= currentInterval) {
            ...
            const coilMapping = this.getCoilMapping();
            return createFanDaoActions(newState, `Fan dao sync: ${newState ? 'ON' : 'OFF'}`, coilMapping);
        }
