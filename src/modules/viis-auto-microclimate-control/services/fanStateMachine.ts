...
export class FanControlStateMachine {
    ...
    private hasTransitionFailed(context: StateMachineContext): boolean {
        const transitionState = this.stateManager.getTransitionState();
    ...
    private canRecoverFromError(context: StateMachineContext): boolean {
        const state = this.stateManager.getState();
    ...
    private shouldEmergencyStop(context: StateMachineContext): boolean {
        const state = this.stateManager.getState();
    ...
    private canExitEmergencyStop(context: StateMachineContext): boolean {
        // Manual intervention required - this would be triggered externally
        return false;
    }

    private hasUnrecoverableError(context: StateMachineContext): boolean {
        const state = this.stateManager.getState();
