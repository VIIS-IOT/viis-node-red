...
describe('Exponential Backoff Retry Logic', () => {
    ...
    beforeEach(() => {
        ...
        mockGlobalContext = {
            get: jest.fn((key: string) => {
                if (key === 'MODBUS_HOLDING_REGISTERS') return { set_flow_A1: 10 };
                if (key === 'MODBUS_COILS') return { pump_1: 0 };
                return {};
            }),
            set: jest.fn()
        };
    ...
    describe('Modbus Write Retry with Backoff', () => {
        test('should retry with exponential backoff delays', async () => {
            ...
            const result = await (scheduleService as any).executeWithExponentialBackoff(
                async () => {
        ...
        test('should fail after max retries with exponential backoff', async () => {
            ...
            await expect(
                (scheduleService as any).executeWithExponentialBackoff(
        ...
        test('should use jitter to prevent thundering herd', async () => {
            ...
            const operations = Array(10).fill(null).map(async (_, index) => {
                ...
                return (scheduleService as any).executeWithExponentialBackoff(
                    () => mockModbusClient.writeRegister(10 + index, 100),
                    2,
                    1000,
                    5000,
                    true // Enable jitter
    ...
    describe('Circuit Breaker Integration', () => {
        test('should open circuit after consecutive failures', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                try {
                    await (scheduleService as any).executeWithCircuitBreaker(
                        () => mockModbusClient.writeRegister(10, 100),
                        circuitKey,
                        5, // Failure threshold
                        30000 // Timeout
            ...
            const circuitState = (scheduleService as any).getCircuitBreakerState(circuitKey);
    ...
    describe('Timeout Protection', () => {
        test('should timeout long-running operations', async () => {
            ...
            await expect(
                (scheduleService as any).executeWithTimeout(
                    () => mockModbusClient.writeRegister(10, 100),
        ...
        test('should return result if operation completes before timeout', async () => {
            ...
            const result = await (scheduleService as any).executeWithTimeout(
                () => mockModbusClient.writeRegister(10, 100),
                5000
            );
        ...
        test('should support custom timeout messages', async () => {
            ...
            await expect(
                (scheduleService as any).executeWithTimeout(
                    () => mockModbusClient.writeRegister(10, 100),
                    1000,
                    'Modbus write operation exceeded 1 second limit'
                )
            ).rejects.toThrow('Modbus write operation exceeded 1 second limit');
    ...
    describe('Combined Resilience Patterns', () => {
        test('should combine timeout, retry with backoff, and circuit breaker', async () => {
            ...
            const result = await (scheduleService as any).executeWithResilience(
                () => mockModbusClient.writeRegister(10, 100),
                {
                    maxRetries: 3,
                    baseDelay: 200,
        ...
        test('should fail fast when circuit is open even with retries configured', async () => {
            ...
            await expect(
                (scheduleService as any).executeWithResilience(
                    () => mockModbusClient.writeRegister(10, 100),
                    {
                        maxRetries: 5,
                        baseDelay: 1000,
