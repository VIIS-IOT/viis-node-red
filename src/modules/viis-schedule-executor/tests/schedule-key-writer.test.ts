import { writeAndVerifyKey } from '../schedule-key-writer';

const baseDeps = () => ({
    writeCoil: jest.fn().mockResolvedValue('written' as const),
    writeRegister: jest.fn().mockResolvedValue(undefined),
    readCoil: jest.fn(),
    readHolding: jest.fn(),
    scaleWrite: (_k: string, v: number) => v,
    scaleRead: (_k: string, v: number) => v,
...
test('holding verify pass uses one write', async () => {
    ...
    const outcome = await writeAndVerifyKey(
...
test('verifyEnabled=false never reads', async () => {
    ...
    const outcome = await writeAndVerifyKey(
        { key: 'set_ec', value: 2.5, fc: 6, unitid: 1, address: 17, quantity: 1 },
        { ...deps, scaleWrite: (_k, v) => v * 1000 },
        { phase: 'set_holding', verifyEnabled: false }
    );
...
test('coil blocked, no retry', async () => {
    ...
    const outcome = await writeAndVerifyKey(
        ...
        deps,
        { phase: 'start_pumps', verifyEnabled: true }
    );
...
test('scale write 2.5 → 2500', async () => {
    ...
    await writeAndVerifyKey(
        { key: 'set_ec', value: 2.5, fc: 6, unitid: 1, address: 17, quantity: 1 },
        { ...deps, scaleWrite: (_k, v) => v * 1000, scaleRead: (_k, v) => v / 1000 },
        { phase: 'set_holding', verifyEnabled: true }
    );
