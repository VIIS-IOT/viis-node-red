...
describe('ScheduleService - Falsy Value Handling', () => {
    ...
    beforeEach(() => {
        ...
        (mockGlobalContextObj.get as jest.Mock).mockImplementation((key: string) => {
            return mockGlobalContext.get(key) || {};
        });
        (mockGlobalContextObj.set as jest.Mock).mockImplementation((key: string, value: any) => {
    ...
    describe('mapScheduleToModbus - Falsy values for unmapped keys (config parameters)', () => {
        ...
        it('should skip all falsy config values: 0, "0", false, "false", null, undefined, ""', () => {
            ...
            expect(result.configParameters).toHaveLength(2);
            expect(result.configParameters.map(p => p.key)).toEqual(
                expect.arrayContaining(['iri_time', 'valid_param'])
            );
        });
        ...
        it('should handle mixed truthy and falsy config values', () => {
            ...
            expect(result.configParameters.map(p => p.key)).toEqual(
                expect.arrayContaining(['iri_time', 'irrigation_mode', 'user_id'])
            );
        });
    });
    ...
    describe('storeConfigParameter - Falsy value handling', () => {
        ...
        it('should return null for value 0 on number key', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key_number', 0, 'test-schedule');
            expect(result).toBeNull();
        });

        it('should coerce 0 to false for boolean key and return null', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key_boolean', 0, 'test-schedule');
            expect(result).toBeNull();
        });

        it('should coerce 1 to true for boolean key and store', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key_boolean', 1, 'test-schedule');
        ...
        it('should return null for value false on boolean key', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key_boolean', false, 'test-schedule');
            expect(result).toBeNull();
        });
        ...
        it('should return null for null value', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key', null, 'test-schedule');
            expect(result).toBeNull();
        });

        it('should return null for undefined value', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key', undefined, 'test-schedule');
            expect(result).toBeNull();
        });

        it('should return null for empty string', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key', "", 'test-schedule');
            expect(result).toBeNull();
        });

        it('should store truthy number values', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key_number', 42, 'test-schedule');
            expect(result).not.toBeNull();
            expect(result).toMatchObject({
                key: 'test_key_number',
                value: 42,
        ...
        it('should store truthy boolean values', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key_boolean', true, 'test-schedule');
            expect(result).not.toBeNull();
            expect(result).toMatchObject({
                key: 'test_key_boolean',
                value: true,
        ...
        it('should store truthy string values', () => {
            const result = (scheduleService as any).storeConfigParameter('test_key_string', "valid_value", 'test-schedule');
            expect(result).not.toBeNull();
            expect(result).toMatchObject({
                key: 'test_key_string',
                value: "valid_value",
