...
const createTestSchedule = (name: string, status: 'running' | 'finished' | ''): TabiotSchedule => ({
    name,
...
describe('Schedule Server Sync Status Tests', () => {
    ...
    beforeEach(() => {
        ...
        scheduleService = new ScheduleService(mockNode, false);
    ...
    describe('updateScheduleStatus - Server Sync', () => {
        it('should log SUCCESS when sync to server succeeds', async () => {
            ...
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('▶️ STATUS CHANGE: test-sync-success')
            );
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('none → running')
            );

            // Verify: Should log sync success
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('✅ SERVER SYNC SUCCESS: test-sync-success')
            );
            expect(mockNode.warn).toHaveBeenCalledWith(
        ...
        it('should log FAILED when sync to server fails', async () => {
            ...
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('⏹️ STATUS CHANGE: test-sync-fail')
            );

            // Verify: Should log sync failure
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('❌ SERVER SYNC FAILED: test-sync-fail')
            );
            expect(mockNode.warn).toHaveBeenCalledWith(
        ...
        it('should log SKIPPED when SyncScheduleService is not available', async () => {
            ...
            const serviceWithoutSync = new ScheduleService(mockNode, false);
            ...
            // Verify: Should log status change
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('STATUS CHANGE: test-sync-unavailable')
            );

            // Verify: Should log sync skipped
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('⚠️ SERVER SYNC SKIPPED: test-sync-unavailable')
            );
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('SyncScheduleService not available')
            );
        });
        ...
        it('should log correct status transition icons', async () => {
            ...
            await scheduleService.updateScheduleStatus(testSchedule, 'running');
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('▶️ STATUS CHANGE')
            );
        ...
        it('should handle database errors gracefully', async () => {
            ...
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('❌ DATABASE ERROR: test-db-error')
            );
            expect(mockNode.warn).toHaveBeenCalledWith(
        ...
        it('should include schedule label in status change log', async () => {
            ...
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('Label: Morning Irrigation')
            );
        });
    });
    ...
    describe('Server Sync - Multiple Status Transitions', () => {
        it('should handle rapid status changes correctly', async () => {
            ...
            const warnCalls = (mockNode.warn as jest.Mock).mock.calls;
            const statusChangeLogs = warnCalls.filter(call => 
                call[0].includes('STATUS CHANGE: test-rapid')
            );
    ...
    describe('Server Sync - Error Recovery', () => {
        it('should continue after sync failure', async () => {
            ...
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('STATUS CHANGE: test-fail-1')
            );
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('STATUS CHANGE: test-success-1')
            );

            // Verify: Failure and success logged
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('❌ SERVER SYNC FAILED: test-fail-1')
            );
            expect(mockNode.warn).toHaveBeenCalledWith(
                expect.stringContaining('✅ SERVER SYNC SUCCESS: test-success-1')
            );
        });
    });
});
