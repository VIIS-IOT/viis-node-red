...
export function formatScheduleRunningRes(inputArray: any) {
    return inputArray.map((item: any) => {
        let {
            name, action, enable, label, interval, set_time,
            start_date, end_date, type, start_time, end_time,
            schedule_plan_id, status, device_id, creation, modified, sp_label,
            sp_start_date, sp_end_date
        } = item;
...
export interface TransformedScheduleBody {
    name: string;
    device_id: string;
    action: string;
    enable: number;
